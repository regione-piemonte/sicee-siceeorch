 ...
 * @generated
 */
public class PAPDGetUidParentFolderRootHandler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDGetUidParentFolderRootHandler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDGetUidParentFolderRootHandler::eseguiFunctionHandler]-binding parametri ");
			...
			LOGGER.debug("[PAPDGetUidParentFolderRootHandler::eseguiFunctionHandler]-richiamo del metodo  " + "getUid");
			it.doqui.index.ecmengine.dto.Node result = pd.getUid(slot_searchParamsIns, slot_operationContextIns);

			LOGGER.debug("[PAPDGetUidParentFolderRootHandler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_rootUid)

			LOGGER.debug(
					"[PAPDGetUidParentFolderRootHandler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_rootUid");
			DataSlotHelper.setDataSlotValue("SLOT_rootUid", result, ps);

		} catch (Exception e) {
			// l'eccezione viene inserita nel contesto per essere processata successivamente
			DataSlotHelper.setLastError(e, ps);
			LOGGER.error(
					"[PAPDGetUidParentFolderRootHandler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione " + e
							+ " nell'apposito slot  ",
					e);
		}
	}
