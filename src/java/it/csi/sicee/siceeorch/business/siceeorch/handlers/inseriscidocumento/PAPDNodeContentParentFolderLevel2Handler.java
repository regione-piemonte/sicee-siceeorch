...
public class PAPDNodeContentParentFolderLevel2Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		try {
			...
			// slot: nodeFolder1Ins
			it.doqui.index.ecmengine.dto.Node slot_nodeFolder1Ins = (it.doqui.index.ecmengine.dto.Node) DataSlotHelper
					.getDataSlotValue("SLOT_nodeFolder1Ins", ps);
			...
			LOGGER.debug("[PAPDNodeContentParentFolderLevel2Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "createContent");
			it.doqui.index.ecmengine.dto.Node result = pd.createContent(slot_nodeFolder1Ins, slot_contentFolderIns,
					slot_operationContextIns);

			LOGGER.debug("[PAPDNodeContentParentFolderLevel2Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_nodeFolder2Ins)

			LOGGER.debug(
					"[PAPDNodeContentParentFolderLevel2Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_nodeFolder2Ins");
			DataSlotHelper.setDataSlotValue("SLOT_nodeFolder2Ins", result, ps);
