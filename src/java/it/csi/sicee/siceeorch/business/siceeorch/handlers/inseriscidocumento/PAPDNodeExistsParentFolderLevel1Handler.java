 ...
 * @generated
 */
public class PAPDNodeExistsParentFolderLevel1Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDNodeExistsParentFolderLevel1Handler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel1Handler::eseguiFunctionHandler]-binding parametri ");
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "nodeExists");
			java.lang.String result = pd.nodeExists(slot_searchParamFolderIns, slot_operationContextIns);

			LOGGER.debug("[PAPDNodeExistsParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_folder1Uid)

			LOGGER.debug(
					"[PAPDNodeExistsParentFolderLevel1Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_folder1Uid");
			DataSlotHelper.setDataSlotValue("SLOT_folder1Uid", result, ps);

		} catch (Exception e) {
			// l'eccezione viene inserita nel contesto per essere processata successivamente
			DataSlotHelper.setLastError(e, ps);
			LOGGER.error(
					"[PAPDNodeExistsParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione "
