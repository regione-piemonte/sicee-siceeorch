 ...
 * @generated
 */
public class PAPDNodeExistsParentFolderLevel2Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDNodeExistsParentFolderLevel2Handler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel2Handler::eseguiFunctionHandler]-binding parametri ");
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel2Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "nodeExists");
			java.lang.String result = pd.nodeExists(slot_searchParamFolderIns, slot_operationContextIns);

			LOGGER.debug("[PAPDNodeExistsParentFolderLevel2Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_folder2Uid)

			LOGGER.debug(
					"[PAPDNodeExistsParentFolderLevel2Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_folder2Uid");
			DataSlotHelper.setDataSlotValue("SLOT_folder2Uid", result, ps);

		} catch (Exception e) {
			// l'eccezione viene inserita nel contesto per essere processata successivamente
			DataSlotHelper.setLastError(e, ps);
			LOGGER.error(
					"[PAPDNodeExistsParentFolderLevel2Handler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione "
