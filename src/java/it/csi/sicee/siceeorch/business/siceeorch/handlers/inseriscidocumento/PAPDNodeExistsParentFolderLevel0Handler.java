 ...
 * @generated
 */
public class PAPDNodeExistsParentFolderLevel0Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDNodeExistsParentFolderLevel0Handler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel0Handler::eseguiFunctionHandler]-binding parametri ");
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel0Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "nodeExists");
			java.lang.String result = pd.nodeExists(slot_searchParamFolderIns, slot_operationContextIns);

			LOGGER.debug("[PAPDNodeExistsParentFolderLevel0Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_folder0Uid)

			LOGGER.debug(
					"[PAPDNodeExistsParentFolderLevel0Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_folder0Uid");
			DataSlotHelper.setDataSlotValue("SLOT_folder0Uid", result, ps);

		} catch (Exception e) {
			// l'eccezione viene inserita nel contesto per essere processata successivamente
			DataSlotHelper.setLastError(e, ps);
			LOGGER.error(
					"[PAPDNodeExistsParentFolderLevel0Handler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione "
