 ...
 * @generated
 */
public class PAPDNodeExistsParentFolderLevel3Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDNodeExistsParentFolderLevel3Handler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel3Handler::eseguiFunctionHandler]-binding parametri ");
			...
			LOGGER.debug("[PAPDNodeExistsParentFolderLevel3Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "nodeExists");
			java.lang.String result = pd.nodeExists(slot_searchParamFolderIns, slot_operationContextIns);

			LOGGER.debug("[PAPDNodeExistsParentFolderLevel3Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_folder3Uid)

			LOGGER.debug(
					"[PAPDNodeExistsParentFolderLevel3Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_folder3Uid");
			DataSlotHelper.setDataSlotValue("SLOT_folder3Uid", result, ps);

		} catch (Exception e) {
			// l'eccezione viene inserita nel contesto per essere processata successivamente
			DataSlotHelper.setLastError(e, ps);
			LOGGER.error(
					"[PAPDNodeExistsParentFolderLevel3Handler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione "
