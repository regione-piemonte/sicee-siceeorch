...
public class PAPDGetUidHandler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		try {
			...
			// slot: searchParamDocIns
			it.doqui.index.ecmengine.dto.engine.search.SearchParams slot_searchParamDocIns = (it.doqui.index.ecmengine.dto.engine.search.SearchParams) DataSlotHelper
					.getDataSlotValue("SLOT_searchParamDocIns", ps);
			...
			it.doqui.index.ecmengine.dto.Node result = pd.getUid(slot_searchParamDocIns, slot_operationContextIns);

			LOGGER.debug("[PAPDGetUidHandler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_nodeUid)

			LOGGER.debug("[PAPDGetUidHandler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
					+ "SLOT_nodeUid");
