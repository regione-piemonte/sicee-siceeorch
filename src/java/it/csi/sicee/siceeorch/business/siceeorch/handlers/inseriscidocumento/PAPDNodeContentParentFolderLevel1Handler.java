...
public class PAPDNodeContentParentFolderLevel1Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		try {
			...
			// slot: nodeFolder0Ins
			it.doqui.index.ecmengine.dto.Node slot_nodeFolder0Ins = (it.doqui.index.ecmengine.dto.Node) DataSlotHelper
					.getDataSlotValue("SLOT_nodeFolder0Ins", ps);
			...
			LOGGER.debug("[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "createContent");
			it.doqui.index.ecmengine.dto.Node result = pd.createContent(slot_nodeFolder0Ins, slot_contentFolderIns,
					slot_operationContextIns);

			LOGGER.debug("[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_nodeFolder1Ins)

			LOGGER.debug(
					"[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
							+ "SLOT_nodeFolder1Ins");
			DataSlotHelper.setDataSlotValue("SLOT_nodeFolder1Ins", result, ps);
