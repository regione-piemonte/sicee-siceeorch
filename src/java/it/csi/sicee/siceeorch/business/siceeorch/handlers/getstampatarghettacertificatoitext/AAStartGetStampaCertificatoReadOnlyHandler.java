package it.csi.sicee.siceeorch.business.siceeorch.handlers.getstampatarghettacertificatoitext;

import java.util.Map;
import org.apache.log4j.Logger;

import com.opensymphony.module.propertyset.PropertySet;
import com.opensymphony.workflow.WorkflowException;

import it.csi.sicee.siceeorch.business.handlers.*;

/**
 * Implementazione dello start node [startGetStampaCertificatoReadOnly].
 * L'implementazione dello start node non aggiunge nessuna logica a quella
 * della superclasse.
 * @generated
 */
public class AAStartGetStampaCertificatoReadOnlyHandler extends StartNodeBaseHandler {
}
