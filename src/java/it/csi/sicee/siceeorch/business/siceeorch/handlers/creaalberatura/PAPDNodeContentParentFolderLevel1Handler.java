 ...
 * @generated
 */
public class PAPDNodeContentParentFolderLevel1Handler extends PAPDCallBaseHandler {
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-binding parametri ");
			// slot: rootUid
			it.doqui.index.ecmengine.dto.Node slot_rootUid = (it.doqui.index.ecmengine.dto.Node) DataSlotHelper
					.getDataSlotValue("SLOT_rootUid", ps);

			// slot: contentFolderLevel1
			it.doqui.index.ecmengine.dto.engine.management.Content slot_contentFolderLevel1 = (it.doqui.index.ecmengine.dto.engine.management.Content) DataSlotHelper
					.getDataSlotValue("SLOT_contentFolderLevel1", ps);
			...
			LOGGER.debug("[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo del metodo  "
					+ "createContent");
			it.doqui.index.ecmengine.dto.Node result = pd.createContent(slot_rootUid, slot_contentFolderLevel1,
					slot_operationContextIns);

			LOGGER.debug("[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_nodeFolderLevel1)

			LOGGER.debug(
					"[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
			...
			LOGGER.error(
					"[PAPDNodeContentParentFolderLevel1Handler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione "
