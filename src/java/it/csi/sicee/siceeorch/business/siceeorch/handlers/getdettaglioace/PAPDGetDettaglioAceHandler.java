 ...
 * - servizio [SICEESRV]
 * - operation [getDettaglioAce]
 * il servizio verra' richiamato tramite configurazione locale della PD
 * @generated
 */
public class PAPDGetDettaglioAceHandler extends PAPDCallBaseHandler {
	...
	/**
	 * Esegue la chiamata all'operazione getDettaglioAce del servizio SICEESRV
	...
	public void eseguiFunctionHandler(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		LOGGER.debug("[PAPDGetDettaglioAceHandler::eseguiFunctionHandler]-START ");
		try {
			...
			LOGGER.debug("[PAPDGetDettaglioAceHandler::eseguiFunctionHandler]-binding parametri ");
			...
			LOGGER.debug(
					"[PAPDGetDettaglioAceHandler::eseguiFunctionHandler]-richiamo del metodo  " + "getDettaglioAce");
			it.csi.sicee.siceesrv.dto.siceesrv.LabelValue[] result = pd.getDettaglioAce(slot_numCertificatore,
					slot_progrCertificato, slot_annoCertificato);

			LOGGER.debug("[PAPDGetDettaglioAceHandler::eseguiFunctionHandler]-richiamo ok  ");

			/// inserimento risultato nello slot apposito (SLOT_array)

			LOGGER.debug("[PAPDGetDettaglioAceHandler::eseguiFunctionHandler]-inserimento del risultato nello slot   "
					+ "SLOT_array");
			...
			LOGGER.error("[PAPDGetDettaglioAceHandler::eseguiFunctionHandler]-richiamo ko, inserimento eccezione " + e
